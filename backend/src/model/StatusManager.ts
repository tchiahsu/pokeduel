  ...
  public static checkIfCanMove(pokemon: Pokemon): StatusResult {
    ...
    switch (status) {
      ...
      case "sleep":
        ...
        if (Math.random() < 0.33 || pokemon.getStatusCounter() == 3) {
          // 33% since the pokemon sleeps 1-3 turns
      ...
      case "freeze":
        ...
        if (Math.random() < 0.2) {
          // 20% chance to get out
          pokemon.setStatus("none");
          return {
            message: `*${pokemon.getName()} thawed out*`,
            canMove: true,
          };
        }
      ...
      case "confuse":
        ...
        if (Math.random() < 0.33) {
          // 33% chance to be confused
          const selfHit = Math.floor(pokemon.getMaxHP() / 8);
          pokemon.takeDamage(selfHit);
          return {
            message: `*${pokemon.getName()} damaged itself in confusion*`,
            canMove: false,
          };
        }
        return { message: null, canMove: true };
      case "paralyze":
        ...
        if (Math.random() < 0.25) {
          ...
          return {
            message: `*${pokemon.getName()} is paralyzed and can't move*`,
            canMove: false,
          };
        }
        return { message: null, canMove: true };
